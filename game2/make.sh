...
case "$1" in
    build)
        case "$2" in
            linux) $CC -Wall -Wextra -pedantic src/{engine_main.c,xlib_window.c,vulkan_renderer.c} -lm -lX11 -lvulkan -o out/game ;;
            windows) $CC -Wall -Wextra -pedantic -target x86_64-windows src/{engine_main.c,win32_window.c,vulkan_renderer.c} -lm -lvulkan-1 -o out/game.exe ;;
